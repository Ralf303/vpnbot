...
- [EMERGENCY_MANAGEMENT.md](EMERGENCY_MANAGEMENT.md) — смена страны без нового файла, аварийное управление из VK/Telegram, добавление VPS и массовое переключение;
...
- Для конфигов московской точки входа пользователь меняет зарубежный выход без перевыпуска файла и сертификата. Для отдельных старых OpenVPN-серверов сохраняется прежний перевыпуск с отзывом старого сертификата через 5 минут.
...
- Меню используют inline-кнопки; текстом вводятся поиск, названия, сроки и данные добавляемого VPS.
- VK поддерживает пользовательские конфиги и аварийное администрирование: добавление VPS, массовую смену выхода и восстановление Telegram. Запуск VK не зависит от доступности Telegram.
- Администратор может массово прибавить 7 дней, 1/3/6 месяцев, 1 год или своё целое количество дней (1–3650) ко всем действующим конфигам с отдельным подтверждением. Число вводится прямо в меню «Массовое продление».
...
- Перед рассылкой выбираются каналы: Telegram, VK или оба. Для VK учитываются связанные аккаунты; при выборе обоих каналов сообщение приходит в каждый. Кнопка файлов работает в обоих мессенджерах.
- Отчёты по Telegram и VK раздельные: количество доставок и ошибок, ID и известные username всех получателей с ошибками. Длинные отчёты разбиваются на сообщения; ошибка одного канала не останавливает другой.
- Файлы `.ovpn` не хранятся в контейнере или базе бота: сервер формирует их по запросу.
- OpenVPN завершается в Москве: российские IPv4-сети выходят напрямую, остальные — через назначенный пользователю WireGuard-выход. Действующий основной выход находится в Германии.
- При отказе выхода администратор переключает пользователей на проверенный резерв без доступа к сломанному серверу. Управляемый зарубежный трафик не переходит автоматически на другой выход; российские назначения продолжают работать.
