...
- Новые клиенты создаются только на новом VPS.
- За 3, 2 и 1 день до окончания бот отправляет по одному напоминанию в 10:00 по Москве.
...
- Старый конфиг продолжает работать без изменений. При повторном получении пользователь подтверждает миграцию: создаётся новый клиент, старый отзывается, срок сохраняется.
